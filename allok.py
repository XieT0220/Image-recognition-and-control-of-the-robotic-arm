...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    # 打开权重文件
    def open_model(self):
        self.openfile_name_model, _ = QFileDialog.getOpenFileName(self.ui.pushButton_weights, 'runs/train/exp14/weights/best.pt',
                                                             'weights/')
            ...
            for i, det in enumerate(pred):
                if det is not None and len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        print(self.names[int(cls)])    #标签
                        symlab = str(self.names[int(cls)])
                        info_show = info_show + str(single_info) + "\n"
        return  info_show
    ...
    def button_image_open(self):
        ...
        address = ('10.44.48.92', 8877)  # 服务端地址和端口10.44.48.92,10.44.74.62
        cli = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        try:
            cli.connect(address)  # 尝试连接服务端
            # cli.send("hello world!".encode())
        except Exception:
            print('[!] Server not found or not open')
            sys.exit()

        # !创建一个 VideoWriter 对象以保存视频
        fourcc = cv2.VideoWriter_fourcc(*'XVID')
        out = cv2.VideoWriter('received_video.avi', fourcc, 20.0, (1280, 720))
        ...
        while True:
            ...
            #                                       defaultButton=QtWidgets.QMessageBox.Ok)
            #     else:
            img_name = "E:/Users/XieT/PycharmProjects/pythonProject/yolov5-5.0 (3)/new_image.jpg"
            img = cv2.imread(img_name)
            print("img_name:", img_name)
            info_show = self.detect(name_list, img)
            print(info_show)
            # print(name_list[0])  #label
            cli.sendall(name_list[0].encode())
            data1 = cli.recv(1024)
            data1 = data1.decode()
            print(2)
            print(data1)
        ...
        # cli.sendall(self.names[int(cls)].encode())
